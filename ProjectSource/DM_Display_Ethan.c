...
bool DM_TakeInitDisplayStep( void )
{
    ...
    case DM_StepStartShutdown:
      ...
      //PSEUDOCODE
      //use send command with each of these to process the bits!
      // in this case we use DM_START_SHUTDOWn (bit 0x0C00)
      //sendCmd(start shutdown) 
    ...
    case DM_StepFillBufferZeros:
      ...
      // we can use a called function to do this
    ...
    case DM_StepDisableCodeB:
     ...
     // using definition: DM_DISABLE_CODEB  0x0900
    ...
    case DM_StepEnableScanAll:
      ...
      //using definition: DM_ENABLE_SCAN= 0x0B07
      //sendcmd(enable the scan)
    ...
    case DM_StepSetBrighness:
      ...
      // move on to next step
      //using definition DM_SET_BRIGHT     0x0A00
      sendCmd(0x0A00);
    ...
    case DM_StepEndShutdown:  
      ...
      //using defined DM_END_SHUTDOWN   0x0C01
      //call it with sendcmd(end shutdown)
        sendCmd(0x0C01);
        
        // now we can exit the loop by changing returnval
        ReturnVal= true;

      //need to reset current step for next time:
      CurrentInitStep= DM_StepStartShutdown;

      //Wait for SS has rise? CHECK THIS COULD BE AN ERROR
...
bool DM_TakeDisplayUpdateStep( void )
{
    ...
    //checks if it's the last row and if so will reset row count and return true else continues count
...
void DM_ScrollDisplayBuffer( uint8_t NumCols2Scroll)
{
    //uint8_t WhichRow;
    // your code goes here
    //we just have to do a shift depending on numcols2scroll:
    
    //CHECK THIS 
    for (int i = 0; i< NUM_ROWS; i++){
        DM_Display[i].FullRow = DM_Display[i].FullRow<< NumCols2Scroll;
    }
}
...
void DM_AddChar2DisplayBuffer( unsigned char Char2Display)
{
    ...
    //for loop through font
    // for rows i= 1 to NUM_ROWS_IN_FONT (~aka 5~)
        //do the action and then add one to the row
    
    //for every row we need to loop through and move each row 
...
void DM_ClearDisplayBuffer( void )
{
  ...
  //for each row clear the display buffer
  for (rowIndex= 0; rowIndex< NUM_ROWS; rowIndex++){
      DM_Display[rowIndex].FullRow = 0x00;
  }
...
{
  bool ReturnVal = true;
  //checking legality of row
  if (0<=WhichRow<=7){
      // adding data into the buffer of the particular row 
...
bool DM_QueryRowData( uint8_t RowToQuery, uint32_t * pReturnValue)
{
  bool ReturnVal = true;
  // test for legal row
  // if RowToQuery is 0 to 7 (~bc that's a legal row~)
    // legal row, so grab the data from the buffer
    // send data to *pReturnvalue = DM_Display[RowToQuery].FullRow 
